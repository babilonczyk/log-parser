...
describe Parser::NonUniqueLogResults do
  ...
  describe 'Get top unique log results' do
    ...
    context 'When sort param is passed and is DESC' do
      ...
      it 'returns array of hashes with pages and their non unique view count in DESC order' do
        expect(subject).to eq(
            [{:count=>5, :page=>"/help_page/1"},
             {:count=>3, :page=>"/home"},
             {:count=>2, :page=>"/contact"},
             {:count=>2, :page=>"/about/2"},
             {:count=>1, :page=>"/about"},
             {:count=>1, :page=>"/index"}]
        )
      end
    ...
    context 'When sort param is passed and is ASC' do 
      ...
      it 'returns array of hashes with pages and their non unique view count in ASC order' do
        expect(subject).to eq(
            [{:count=>1, :page=>"/index"},
             {:count=>1, :page=>"/about"},
             {:count=>2, :page=>"/about/2"},
